...
namespace GhysX.Framework.Utilities
{
    public class FileUtils
    {
		...
		{
			//~~ if (path.IndexOf(Constants.StreamingAssets) != -1 && Constants.UpdateMode)
			// {
			// 	if (File.Exists(path))
			// 	{
			// 		File.Delete(path);
			...
			// 	}
			// }
			// if ((!File.Exists(path) && path.IndexOf(Constants.StreamingAssets) == -1) || !Constants.UpdateMode)
			// {
			...
			// catch (Exception ex)
			// {
			// 	Debug.LogError("Can't create local resource" + path);
			// 	return false;
			// }
			return true;
		}
